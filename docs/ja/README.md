...
必ずしもこれら全部を最初に読む必要はないのですが、もしお役に立てば。
...
- `package.json` ファイルは、`npm` あるいはその他のパッケージ管理ツールを使うのに必要です。これには、パッケージ間の依存関係の情報などといった各種メタデータが書かれています。また、ユーザー定義のスクリプトが含まれていることもあります（下記「npm script」も参照）。
...
ただしこれは、このページに書いてあるトピックが十分に簡単であることを意味しません。  
何かを学ぶというのは時間がかかるものです。
